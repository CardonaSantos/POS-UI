...
type KpiType = "FACTURACION" | "CLIENTE_SERVICIO" | "CLIENTE_COBRANZA";
  ...
  return (
    ...
    >
      <article
        className={cn(
          [
            "flex h-[42px] min-w-0 flex-col justify-between overflow-hidden",
            "rounded-[var(--app-radius-md)] border",
            "px-2 py-1.5",
            "transition",
      ...
      >
        <div className="flex min-w-0 items-center justify-between gap-1">
          <p className="truncate text-[8px] font-semibold uppercase leading-none tracking-wide text-[hsl(var(--app-muted-foreground,var(--muted-foreground)))]">
            {title}
          </p>
        ...
        <p
          title={String(value)}
          className="truncate text-[13px] font-semibold leading-none tabular-nums text-[hsl(var(--app-foreground,var(--foreground)))]"
        >
          {value}
        </p>
      </article>
...
function getKpiHref(type: KpiType, linkValue: string) {
  if (type === "FACTURACION") {
    if (!linkValue) {
      return "/crm/facturacion";
    }

    return `/crm/facturacion?estadoFactura=${encodeURIComponent(linkValue)}`;
  }

  if (type === "CLIENTE_SERVICIO") {
    if (!linkValue) {
      return "/crm-clientes";
    }

    return `/crm-clientes?estado=${encodeURIComponent(linkValue)}`;
  }
  ...
  return `/crm-clientes?estadoCobranza=${encodeURIComponent(linkValue)}`;
}
